...
def configurer_bgp_policies(intent):
    ...
    for role, comm in communities.items():
        cfg += f"ip community-list standard {role.upper()} permit {comm}\n" #ermet au routeur de permettre les routes qui ont la communauté comm et de les traiter selon les politiques définies pour la liste de communautés role
    cfg += "\n"
...
!
""" # Crée une règle dans la route-map qui autorise les routes et leur attribue un numéro de priorité (10).
    #ensuite Ajoute la communauté comm à la route sans enlever les communautés existantes.
    #enfin on modifie la préférence locale de la route à lp, influençant ainsi sa priorité pour le routeur.
...
!
""" #définit une route-map appelée RM-SET-LOCAL avec une règle prioritaire (10)
    #définit une préférence locale de local_pref['customer'] pour les routes qui correspondent à cette règle, influençant ainsi leur sélection parmi d'autres chemins BGP.
    #ajoute la communauté communities['customer'] à ces routes, ce qui peut être utilisé pour appliquer des politiques spécifiques ou pour marquer les routes pour un traitement ultérieur.
    ...
    for to_key, allowed_roles in policy.items():
        ...
        for r in allowed_roles:
            cfg += f"ip community-list standard {listname} permit {communities[r]}\n" #définit une liste de communautés avec le nom listname (par exemple, "TO_PEER") et autorise les communautés communities[r]
        cfg += "\n"
...
!
""" #génère une route-map BGP pour la propagation des communautés, en l'appelant RM-OUT-TO-{target}avec règle prioritaire DE 10
    #ensuite on spécifie que cette route-map matche les communautés dans la liste de communautés définie plus haut (listname) Cela indique que cette règle est appliquée aux routes qui ont les communautés spécifiées dans listname.
    #définit une règle supplémentaire dans la même route-map pour nier toute autre route qui ne correspond pas à la règle précédente. numéro "20" indique que c'est une règle avec une priorité plus basse (appliquée après la règle "permit 10").
    return cfg
...
""" #route-map RM-IN-{role.upper()} in : Applique la route-map d'entrée en fonction du rôle (par exemple, "customer" ou "peer"). Cela permet de filtrer ou de modifier les routes reçues en fonction de la relation avec ce voisin.
    #route-map RM-OUT-TO-{role.upper()} out : Applique la route-map de sortie en fonction du rôle, contrôlant les routes envoyées à ce voisin.
    #send-community : Assure que les communautés BGP sont envoyées au voisin.
    #essai de correction du next hop/routes
